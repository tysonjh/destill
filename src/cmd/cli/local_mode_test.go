...
// TestBuildAnalysisRequest tests request creation
func TestBuildAnalysisRequest(t *testing.T) {
	buildURL := "https://buildkite.com/myorg/pipeline/builds/123"

	requestID, data, err := buildAnalysisRequest(buildURL)
	if err != nil {
		t.Fatalf("buildAnalysisRequest() unexpected error: %v", err)
	}

	// Test 1: Request ID should be set
	if requestID == "" {
		t.Error("buildAnalysisRequest() RequestID is empty")
	}

	// Test 2: Data should not be empty
	if len(data) == 0 {
		t.Error("buildAnalysisRequest() Data is empty")
	}
	...
	if err := json.Unmarshal(data, &payload); err != nil {
		t.Fatalf("buildAnalysisRequest() Data is not valid JSON: %v", err)
	}

	// Test 4: JSON payload should match request fields
	if payload.RequestID != requestID {
		t.Errorf("buildAnalysisRequest() JSON RequestID = %s, want %s", payload.RequestID, requestID)
	}
	if payload.BuildURL != buildURL {
		t.Errorf("buildAnalysisRequest() JSON BuildURL = %s, want %s", payload.BuildURL, buildURL)
	}
}
...
func TestLocalMode(t *testing.T) {
	t.Run("NewLocalMode creates valid instance", func(t *testing.T) {
		...
		if mode.Broker() == nil {
			t.Error("NewLocalMode() Broker() returned nil")
		}
	})
	...
	t.Run("SubmitAnalysis returns valid request ID", func(t *testing.T) {
		...
		buildURL := "https://buildkite.com/myorg/pipeline/builds/123"
		requestID, err := mode.SubmitAnalysis(buildURL)
		if err != nil {
			t.Fatalf("SubmitAnalysis() unexpected error: %v", err)
		}

		if requestID == "" {
			t.Error("SubmitAnalysis() returned empty requestID")
		}

		if !strings.HasPrefix(requestID, "req-") {
			t.Errorf("SubmitAnalysis() requestID doesn't start with 'req-': %s", requestID)
		}
	})
