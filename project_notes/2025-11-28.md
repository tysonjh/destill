...
## feat: implement Bubble Tea TUI for triage reporting

Built an interactive Terminal User Interface (TUI) using Bubble Tea and lipgloss to display analyzed failure cards. The TUI validates Phase 2's intelligence by presenting confidence scores, recurrence tracking, and log snippets in an elegant table view.

### Summary

The Triage Reporter TUI provides an interactive interface for viewing and navigating failure cards ranked by confidence and recurrence. This completes Phase 3 of the development roadmap and brings the complete stream processing pipeline to life in a user-friendly terminal interface.

#### 1. TUI Package (`src/tui/`)
- **`triage.go`**: Complete Bubble Tea model implementation
  - `TriageModel`: Accepts pre-sorted slice of `TriageCard`
  - `Init()`: Initializes the Bubble Tea model
  - `Update()`: Handles keyboard navigation (↑/↓, j/k for vim-style, q/ctrl+c to quit)
  - `View()`: Renders styled table with lipgloss
  - Column display: Confidence (0.00-1.00), Recurrence count, Hash (first 8 chars), Log snippet (80 chars)
  - Cursor highlighting with reverse colors
  - Empty state handling
  - Help text footer

- **`triage_test.go`**: Comprehensive unit tests
  - Model initialization tests
  - Empty data handling tests
  - Navigation boundary tests (up/down/j/k)
  - View rendering tests
  - Metadata extraction tests

#### 2. CLI Integration (`src/cmd/cli/`)
- **Updated `main.go`**:
  - Added Bubble Tea and TUI package imports
  - `analyzeCmd`: Collects triage cards from `ci_failures_ranked` topic
  - `collectTriageCards()`: Subscribes to broker and gathers cards for 5 seconds
  - `getRecurrenceCount()`: Helper function to extract recurrence metadata
  - Sorts cards by confidence score (descending), then by recurrence count
  - Launches TUI with `tea.NewProgram()` in alternate screen mode

#### 3. Demo Program (`demos/`)
- **`tui_demo.go`**: Standalone demo showcasing TUI with realistic sample data
  - 5 sample triage cards with varied severity levels (FATAL/ERROR/WARN)
  - Realistic failure messages (OutOfMemoryError, connection timeouts, test assertions, etc.)
  - Varied recurrence counts (1-15)
  - Confidence scores ranging from 0.75 to 0.98
...
- **Boundary Checking**: Cursor cannot move beyond first or last item
- **Vim-Style Navigation**: Support for both arrow keys and vim keybindings

#### Styled Table View
- **Header Row**: Bold, bright blue text on dark gray background
- **Data Columns**:
  - Confidence: Displayed as floating-point percentage (e.g., 0.95)
  - Recurrence: Count from metadata or default to 1
  - Hash: First 8 characters for brevity
  - Log Snippet: Truncated to 80 characters with ellipsis
- **Selection Highlight**: Current row shown with reversed colors
- **Help Text**: Faint gray footer with navigation instructions

#### Data Integrity
- Pre-sorted input: Cards must be sorted before passing to TUI
- Immutable display: TUI does not modify card order
- Metadata preservation: All fields passed through from analysis agent
- Empty state handling: Graceful message when no failures detected

### Modified Files
- `src/tui/triage.go` - New TUI model implementation
- `src/tui/triage_test.go` - Comprehensive test coverage
- `src/cmd/cli/main.go` - TUI integration and card collection logic
- `demos/tui_demo.go` - Standalone demonstration program

### Success Criteria

- [x] TUI model accepts pre-sorted TriageCard slice
- [x] Table view displays confidence, recurrence, hash, and snippet
- [x] Navigation works with arrow keys and vim bindings (j/k)
- [x] Cursor highlighting with lipgloss reverse style
- [x] Boundary checking prevents navigation beyond list edges
- [x] Empty state displays helpful message
- [x] Exit functionality with q and ctrl+c
- [x] All tests passing (100% coverage for navigation and view rendering)
- [x] Application builds successfully
- [x] Demo program showcases real-world scenarios

### Status

✅ **Complete - Interactive TUI operational**

The Triage Reporter TUI successfully validates Phase 2's intelligence with an elegant, interactive interface for exploring failure patterns!

### Dependencies Added
- `github.com/charmbracelet/bubbletea@v1.3.10` - TUI framework
- `github.com/charmbracelet/lipgloss@v1.1.0` - Terminal styling (included with bubbletea)

### Usage

Run the demo to see the TUI in action:
```bash
go run demos/tui_demo.go
```

Or integrate with the full pipeline:
```bash
export BUILDKITE_API_TOKEN="your-token-here"
./destill build https://buildkite.com/org/pipeline/builds/4091
./destill analyze  # Wait 5 seconds for cards to collect, then TUI launches
```
...
- **Phase 5**: Multi-platform support (GitHub Actions, GitLab CI)
- **Phase 6**: ML-based pattern learning and classification
- **Phase 7**: Enhanced TUI features (filtering, search, detail view, export)
