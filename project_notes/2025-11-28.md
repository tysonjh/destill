...
Improved pattern matching with multiple levels:
- **FATAL**: `fatal`, `panic`, `segmentation fault`, `core dumped`, `out of memory`
- **ERROR**: `error`, `exception`, `failed`, `failure`, `cannot`, `unable to`, `denied`
...
- **`TestDetectSeverityEnhanced`**: Tests multi-level severity classification
- **`TestCalculateConfidenceScore`**: Validates dynamic confidence scoring
